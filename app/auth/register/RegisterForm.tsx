...
const roleOptions = [
  { value: 'buyer' as Role, label: 'Buy or Rent Property',    desc: 'Browse listings, save favorites, and purchase properties safely.', icon: 'fa-solid fa-house-circle-check' },
  { value: 'agent' as Role, label: 'List & Sell Properties',  desc: 'Create listings, manage clients, and earn commissions.',            icon: 'fa-solid fa-briefcase' },
]
          ...
          {step === 1 ? (
            <div>
              ...
              <button onClick={() => setStep(2)} className="w-full btn-primary py-3 text-base justify-center">
                Continue <i className="fa-solid fa-arrow-right text-sm" />
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Chidi Okeke" className="input-base pl-10" autoComplete="name" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="input-base pl-10" autoComplete="email" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters" className="input-base pl-10 pr-10" autoComplete="new-password" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--color-text-muted)' }}>
              ...
              <div>
                ...
                <div className="relative">
                  <i className="fa-solid fa-lock absolute left-3.5 top-1/2 -translate-y-1/2 text-xs" style={{ color: 'var(--color-text-muted)' }} />
                  <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Repeat password" className="input-base pl-10" autoComplete="new-password" />
                </div>
              </div>
